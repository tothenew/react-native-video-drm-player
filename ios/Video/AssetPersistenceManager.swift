...
public class AssetPersistenceManager: NSObject {
    ...
    @objc public static let sharedManager = AssetPersistenceManager()
...
//                [AVAssetDownloadTaskMinimumRequiredMediaBitrateKey: 265_000]) else { return }
  ...
  @objc func localAssetForStream(withName name: String) -> AVURLAsset? {
    ...
    @objc
    public func downloadState(videoName:String) -> Asset.DownloadState {
        if let localFileLocation = localAssetForStream(withName: videoName)?.url {
    ...
    @objc
    public func getAssetOfflineModel(videoName:String) -> Asset? {
        var asset: Asset? = nil;
        if let avUrlAsset = localAssetForStream(withName: videoName) {
            // Check if the file exists on disk
            if FileManager.default.fileExists(atPath: avUrlAsset.url.path) {
                let stream: Stream = Stream.init(name: videoName);
                asset = Asset.init(stream: stream, urlAsset: avUrlAsset);
            }
        }
        return asset;
    }
